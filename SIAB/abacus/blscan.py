...
def blgen(elem: str,
    ...
    '''
    from SIAB.data.build import AtomSpecies, CellGenerator
    
    call = {'dimer': CellGenerator.get_dimer_bond_length,
            'trimer': CellGenerator.get_trimer_bond_length}
    
    OrbgenAssert(proto in call.keys(), 'Unknown prototype: %s'%proto)

    l_bnd = call[proto](elem)
    if l_bnd is not None:
        logging.info(f'Prototype {proto} is found, use the default bond length {l_bnd}.')
        return l_bnd
    else:
        logging.info(f'Prototype {proto} is not found, perform bond length scan task.')
        bl0 = AtomSpecies.get_covalent_radius(elem) * 2 * 0.8
...
def _blgrid_gen(center: float, 
    ...
    logging.info(f'Searching bond lengths from {blmin:4.2f} to {blmax:4.2f} Angstrom.')
...
which indicates unreasonable bond length sampling.')
    OrbgenAssert(idx_min < len(e) - 2,
                 'There are fewer than 2 points on the right side of the minimum energy point, \
which indicates unreasonable bond length sampling.')
    OrbgenAssert(len(r) >= 5, 'There are fewer than 5 points in total.')
    ...
    while not conditioned:
        ...
        logging.info('MORSE POTENTIAL FITTING: remove the highest energy point %4.2f eV \
at bond length %4.2f Angstrom.'%(e[idx_remove], r[idx_remove]))
        e.pop(idx_remove)
        r.pop(idx_remove)
    ...
    try:
        ...
        OrbgenAssert(False, 'Fitting failed. This may due to the ill-defined PES \
caused by low-quality pseudopotential')
    ...
    logging.info(f'{"D_e":6s}: {popt[0]:15.10f} {"eV":10s} (Bond dissociation energy)')
    logging.info(f'{"a":6s}: {popt[1]:15.10f} {"":10s} (Morse potential parameter)')
    logging.info(f'{"r_e":6s}: {popt[2]:15.10f} {"Angstrom":10s} (Equilibrium bond length)')
    logging.info(f'{"e_0":6s}: {popt[3]:15.10f} {"eV":10s} (Zero point energy)')
...
def _select(bl0: float, 
    ...
    if n // 2 == 0:
        n = n + 1
        logging.info(f'Number of bond lengths to select must be odd, reset to {n}.')
    OrbgenAssert(n <= len(bl), 
                 'Number of bond lengths to select is larger than the number \
of bond lengths provided.')

    idx = np.argsort(bl)
    bl, e = np.array(bl)[idx], np.array(e)[idx]

    OrbgenAssert(bl0 >= bl[0] and bl0 <= bl[-1],
                 'Equilibrium bond length is out of the range of bond lengths.')
    
    e = e - e0 # shift the energy to zero point energy
    OrbgenAssert(all(e >= 0), 
                 'Ill-defined PES: there are points with energies lower than zero point energy.')
    ...
    while e[left] - e[center] > ethr:
        left += 1
    OrbgenAssert(center - left >= n // 2,
                 'Not enough bond lengths on the left side of the minimum energy point.')
    left = np.linspace(left, center, n//2+1).astype(int)

    right = len(bl) - 1
    while e[right] - e[center] > ethr:
        right -= 1
    OrbgenAssert(right - center >= n // 2,
                 'Not enough bond lengths on the right side of the minimum energy point.')
    ...
    for c in os.listdir(root):
        ...
        if m is not None:
            ...
            try:
                ...
                logging.error(f'Error in reading energy for {c}: {e}')
...
class TestBLSCAN(unittest.TestCase):
    ...
    def test_jobfilter(self):
        ...
        bl = np.linspace(1.0, 3.0, 11).tolist()
        e = _morse(bl, 1.0, 1.0, 2.0, -0.5) # model data
        candidates = []
