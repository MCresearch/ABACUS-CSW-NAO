...
def _radii_interp(element: str, radius_data: dict):
    ...
    if element not in radius_data.keys():
        left = ELEMENT_TO_INDEX[element]
        while True:
            left -= 1
            if INDEX_TO_ELEMENT[left] in radius_data.keys():
                left = INDEX_TO_ELEMENT[left]
                break
            if left < 1:
                raise ValueError('Element %s not in database, interpolation is also not possible' % element)
        right = ELEMENT_TO_INDEX[element]
        while True:
            right += 1
            if INDEX_TO_ELEMENT[right] in radius_data.keys():
                right = INDEX_TO_ELEMENT[right]
                break
            if right > 118:
                raise ValueError('Element %s not in database, interpolation is also not possible' % element)
        '''linear interpolation'''
        distance_left = ELEMENT_TO_INDEX[element] - ELEMENT_TO_INDEX[left]
        distance_right = ELEMENT_TO_INDEX[right] - ELEMENT_TO_INDEX[element]
