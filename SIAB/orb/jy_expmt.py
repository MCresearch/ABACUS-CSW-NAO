...
from SIAB.spillage.lcao_wfc_analysis import \
    typewise_wavefunction_angular_momentum_decomposition
from SIAB.spillage.spillage import flatten, initgen_jy

def _band_l_indexing(folder, 
                     count_thr = 1e-1, 
                     itype = 0):
    '''
    ...
    '''
    OrbgenAssert(itype == 0, "Currently only supports itype = 0")
    ...
    if any([not os.path.exists(os.path.join(folder, f)) 
            for f in [fwfc + '1.txt', frunning]]) else folder
    ...
    OrbgenAssert(lmax_it >= 0, f"lmax for one atomtype should be at least 0: {lmax_it}")
    ...
    for isk in range(nspin*len(wk)): 
        ...
        wll = typewise_wavefunction_angular_momentum_decomposition(
            wfc, ovlp, running["natom"], running["nzeta"])
...
def _ibands(ibnd_max, igeom, npert, ibnd_min = None):
    ...
    OrbgenAssert(all([isinstance(i, int) for i in ibnd_max]), 
                 "ibnd_max should be int or list of int")
    igeom = [igeom] if not isinstance(igeom, list) else igeom
    OrbgenAssert(all([isinstance(i, int) for i in igeom]), 
                 "igeom should be int or list of int")
    ibnd_min = [0] * len(ibnd_max) if ibnd_min is None else ibnd_min
    ibnd_min = [ibnd_min] if not isinstance(ibnd_min, list) else ibnd_min
    OrbgenAssert(all([isinstance(i, int) for i in ibnd_min]), 
                 "ibnd_min should be int or list of int")
    OrbgenAssert(len(ibnd_min) == len(ibnd_max), 
                 "ibnd_min and ibnd_max should have the same length")
    OrbgenAssert(len(ibnd_max) == len(npert), 
                 "ibnd_max and npert should have the same length")

    return flatten([[range(ibmin, ibmax)]*npert[ig] 
                    for ig, ibmax, ibmin in zip(igeom, ibnd_max, ibnd_min)])

def _coef_init(outdir, nzeta, izmin = None, diagnosis = False, lloc_min = 4):
    ...
    lloc_min : int
        the minimal l that would be initialized by the auxiliary potential rather
        than this function, default is 4
    
    Returns
    -------
    list[list[list[float]]]: the initial guess of all coefficients, indexed by
    [l][n][q]'''
    # HARD-CODED: only one spin
    _bandmap = _band_l_indexing(outdir)[0] # indexed by [l][n] -> list[int]
    izmin = [0] * len(nzeta) if izmin is None else izmin
    OrbgenAssert(len(izmin) == len(nzeta), 
                 "izmin and nzeta should have the same length")
    # ensure there are enough bands for the calculation
    if len(nzeta) > lloc_min:
        logging.warning(f'components whose angular momentum l >= {lloc_min} '
                        'will not be initialized by default. Instead, it is '
                        'expected to use the one from solving radial Schrodinger '
                        'equantion, once the parameter `vloc_aux` is set. '
                        'Otherwise, specifying the `lloc_min` to higher value will '
                        'continue the `atomic` initialization.')
    check = [len(_bandmap[l]) >= nz for l, nz in enumerate(nzeta) if l < lloc_min]
    OrbgenAssert(all(check) or len(check) == 0, 
        'initgen error: requiring more bands than actually calculated for '
        f'angular momentum l = {[l for l, c in enumerate(check) if not c]}')

    coef = [[] for _ in nzeta]
    for l, nzl in enumerate(nzeta):
        if l < lloc_min: # otherwise the auxiliary potential will be used
            for iz in range(izmin[l], nzl):
                ib = _bandmap[l][iz] # band to analysis, but how to deal with the spin?...
                nz = np.array([0 if i != l else 1 for i in range(len(nzeta))])
                c = initgen_jy(outdir, nz, ib, diagnosis=diagnosis)
                coef[l].append(c[l][0])
    return coef

class TestSpillageExperimental(unittest.TestCase):

    def test_band_l_indexing(self):
        here = os.path.dirname(__file__)
        parent = os.path.dirname(here)
        fpath = os.path.join(parent, "spillage/testfiles/Si/jy-7au/monomer-gamma/OUT.ABACUS")
        out = _band_l_indexing(fpath)
