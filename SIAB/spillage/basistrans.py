...
def jy2ao(coef, natom, nbes):
    ...
    OrbgenAssert(len(natom) == len(nbes) == len(coef),
        f'Inconsistent number of atom types: {len(natom)}, {len(nbes)}, {len(coef)}')

    # 2. len(coef[itype]) (number of l) should not exceed len(nbes[itype]).
    OrbgenAssert(all(len(coef_t) <= len(nbes_t) 
                     for nbes_t, coef_t in zip(nbes, coef)),
        'len(coef[itype]) (number of l) should not exceed len(nbes[itype])')

    # 3. len(coef[itype][l][zeta]) should not exceed nbes[itype][l].
    OrbgenAssert(all(all(all(len(coef_tlz) <= nbes_tl for coef_tlz in coef_tl)
                   for nbes_tl, coef_tl in zip(nbes_t, coef_t))
                 for nbes_t, coef_t in zip(nbes, coef)),
        'len(coef[itype][l][zeta]) should not exceed nbes[itype][l]')
