...
def read_orb_mat(fpath):
    ...
    OrbgenAssert(len(ref_ref) == nbands * nk, 
                 f'Length of ref_ref ({len(ref_ref)}) != nbands*nk ({nbands*nk})')
...
def read_csr(fname):
    ...
    OrbgenAssert((data[0].startswith('STEP: 0') \
...
def read_triu(fname):
    ...
    sz = int(data[0])
    OrbgenAssert(len(data) == sz*(sz+1)//2 + 1 or len(data) == sz*(sz+1) + 1,
        'the length of the data is not consistent with the size of the matrix')
...
def read_istate_info(fname):
    ...
    OrbgenAssert(all([re.match(title, line) for line in kpt]),
        'the kpoint line is not in the correct format')
