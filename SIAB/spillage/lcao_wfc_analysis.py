...
def api(C, S, natom, nzeta, method, **kwargs):
    ...
    '''
    OrbgenAssert(method in ['twsvd', 'amwsvd'], 'method not recognized')
    nband = kwargs.get('nband', 'all')
    threshold = kwargs.get('threshold', None)
    wsvd = {
        'twsvd': typewise_wavefunction_singular_value_decomposition,
        'amwsvd': atomwise_maximum_wavefunction_singular_value_decomposition
        }
    return wsvd[method](C, S, nband, natom, nzeta, threshold)

def typewise_wavefunction_angular_momentum_decomposition(
    C, S, natom, nzeta):
    '''
...
def atomwise_maximum_wavefunction_singular_value_decomposition(
    C, S, nband, natom, nzeta, threshold = None):
    ...
    ##############################################
    threshold = [threshold] * ntyp if isinstance(threshold, float) else threshold
    jobtype = 'nzeta-infer-task' \
        if isinstance(threshold, list) \
           and len(threshold) == ntyp \
           and all(isinstance(f, float) for f in threshold) \
        else 'loss-esitmation-task'

    nz = [[len(np.where(np.array(sigma) >= threshold[it])[0]) 
           for sigma in out[it]] for it in range(ntyp)] \
         if jobtype == 'nzeta-infer-task'\
         else threshold # jobtype == 'loss-esitmation-task'
        ...
        for l in range(lmax[it]+1):
            for ia in range(natom[it]):
                for m in range(2*l+1):
                    ...
                    loss += la.norm(u[:, nz[it][l]:] @ \
                            np.diag(out[it][l][nz[it][l]:]) @ \
                            vh[nz[it][l]:, :], 'fro')**2
    return out, nz, loss

def typewise_wavefunction_singular_value_decomposition(
    C, S, nband, natom, nzeta, threshold = None):
    ...
    nzeta_sub: list[list[int]]|None
        the number of zeta functions after filtering, can be indexed by [it][l].
        if `threshold` is not specified, will be None.
    loss: list[float]|None
        loss of space truncation, can be indexed by [it]. If `threshold` is not
        specified, will be None.
    '''
    ...
    nz = [[len(np.where(np.array(sigma_l) >= threshold[it] * np.sqrt((2*l+1)*natom[it]))[0])
           for l, sigma_l in enumerate(sigma[it])] \
           for it in range(ntyp)] \
           if jobtype == 'nzeta-infer-task' \
         else threshold # jobtype == 'loss-esitmation-task'
...
class TestLCAOWfcAnalysis(unittest.TestCase):
    ...
    def test_twamd_multi_k(self):
        ...
        wll = typewise_wavefunction_angular_momentum_decomposition(wfc, S, dat['natom'], dat['nzeta'])
    ...
    def test_nzeta_nband_plt(self):
        ...
        outdir = '/root/documents/simulation/orbgen/Test1Aluminum-20241011/Al-dimer-2.00-10au/OUT.Al-dimer-2.00-10au/'
        analyzer = typewise_wavefunction_singular_value_decomposition
        # analyzer = atomwise_maximum_wavefunction_singular_value_decomposition
        wfc = read_wfc_lcao_txt(outdir + 'WFC_NAO_GAMMA1.txt')[0]
        S = read_triu(outdir + 'data-0-S')
        dat = read_running_scf_log(outdir + 'running_scf.log')
    ...
    def test_amwsvd(self):
        ...
        sigma, nz, loss = atomwise_maximum_wavefunction_singular_value_decomposition(
            wfc, S, 'all', dat['natom'], dat['nzeta'])
    ...
    def test_twsvd(self):
        ...
        sigma, nz, loss = typewise_wavefunction_singular_value_decomposition(
            wfc, S, 'all', dat['natom'], dat['nzeta'])
