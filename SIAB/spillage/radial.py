...
def vlocal(vgrid, v, r, chi1, chi2):
    '''
    "local" potential (always is of the semi-local pseudopotential of l = 4)
    energy of pseudo-atomic orbitals.
    
    The energy of a pseudo-atomic orbital

                phi(vec{r}) = chi(r) * Y_{lm}(hat{r})
                
    merely depends on the radial part chi(r) and the local pseudopotential
    v(r). Given a radial function chi(r) evaluated on grid r and the
    semilocal pseudopotential v(r), this function evaluates the integral

        / rcut
        |      dr chi1(r) * v(r) * chi2(r) * r^2
        / 0
    ...
    -------
        float
            Local potential energy of the pseudo-atomic orbital.
    '''
    v = CubicSpline(vgrid, v, extrapolate=True) # extrapolate is necessary
    return simpson(r**2 * chi1 * v(r) * chi2, x=r)
...
def proj_jl(radial, dr, l, ecut, rcut, primitive_type: str = 'reduced'):
    '''
    ...
    orb = Orbital(rcut, ecut, 'any', [0]*(l+1), primitive_type)
    orb.coef_ = orb.coefgen('ones')
    chi = np.array(orb.to_griddata(dr=dr)[l])
...
class _TestRadial(unittest.TestCase):
    ...
    def test_vlocal(self):
        ...
        vloc = np.ones(nr) # so that it is the overlap
        nq = 5
        l = 3
        
        mat = np.zeros((nq, nq))
        for i, q1 in enumerate(range(nq)):
            for j, q2 in enumerate(range(nq)):
                mat[i, j] = vlocal(r, vloc, r, jl_raw(l, q1, r, rcut),
                                   jl_raw(l, q2, r, rcut))
        nonzeros = [mat[i, i] for i in range(nq)]
        self.assertTrue(np.allclose(mat, np.diag(nonzeros)))
