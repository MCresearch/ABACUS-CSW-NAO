...
def _initgen_core(nzeta, nbes_data, ref_jy, wk, nbes_gen, diagnosis):
    ...
    for l, nzeta_l in enumerate(nzeta):
        ...
        if diagnosis:
            logging.info( "initgen: <jy|ref><ref|jy> eigval diagnosis:")
            logging.info(f"         l = {l}: {val[-nzeta_l:][::-1]}")
...
def initgen_file(forb, nzeta, ecut, rcut, dr = 0.01, primitive_type='reduced'):
    '''
    ...
    for l, (chi, nz) in enumerate(zip(nao['chi'], nzeta)):
        coef_l = [proj_jl(chi[i], dr, l, ecut, rcut, primitive_type)[0].tolist()
                  for i in range(nz)]
        coef.append(coef_l)
...
def initgen_vloc(fn, l, nzeta, ecut, rcut, primitive_type='reduced', dr=0.01):
    '''
    get coefficients of `n` zeta functions with angular momentum `l` from the
    file containing the local potential.
    
    Parameters
    ----------
    fn: str
        the path to the local potential file (always the pseudopotential file)
    l: int|list[int]
        the angular momentum(s) of the zeta functions
    nzeta: int|list[int]
        the number of zeta functions to init
    ecut: float
        the kinetic energy cutoff of the jy basis that used to generate new orbitals
    rcut: float
        the cutoff radius of the jy basis that used to generate new orbitals
    primitive_type: str
        the type of jy basis, can be `reduced` or `normalized`
    dr: float
        the step size of the radial grid, in Bohr, default is 0.01
    ...
    Warning
    -------
    It is only recommended to use this function for l >= 4 cases, because the local
    potential in pseudopotential file is always the semi-local potential in radial
    Schrodinger equation of l = 4.
    '''
    # third-party modules
    import numpy as np
    
    # local modules
    from SIAB.spillage.radial import kinetic, vlocal
    from SIAB.orb.orb import Orbital
    from SIAB.io.psp import PspParser
    
    def _solve(vgrid, v, rgrid, bas, l, nroot):
        '''
        solve the radial Schrodinger equation on the given numerical
        radial basis set of which the angular momentum is l, get n
        the lowest eigenvalues and eigenvectors.
        
        Parameters
        ----------
            vgrid : np.ndarray
                Radial grid of the pseudopotential.
            v : np.ndarray
                Pseudopotential evaluated on the radial grid vgrid.
            rgrid : np.ndarray
                Radial grid.
            bas : list of np.ndarray
                numerical radial basis set whose angular momentum is l.
            l : int
        ...
        # kinetic energy is diagonal in the {|r>} space
        T = np.diag([kinetic(rgrid, l, chi) for chi in bas])
        # local potential energy matrix
        # (can only calculate the upper or lower triangle)
        V = np.zeros((len(bas), len(bas)))
        for i in range(len(bas)):
            for j in range(i, len(bas)):
                V[i, j] = vlocal(vgrid, v, rgrid, bas[i], bas[j])
                V[j, i] = V[i, j]
        
        # solve the radial Schrodinger equation
        eigvals, eigvecs = eigh(T + V)
        return eigvals[:nroot], eigvecs[:, :nroot]
    
    l = [l] if isinstance(l, int) else l
    nzeta = [nzeta]*len(l) if isinstance(nzeta, int) else nzeta
    assert len(l) == len(nzeta), 'l and nzeta should have the same length'
    
    orb = Orbital(rcut, ecut, 'any', [0]*(max(l)+1), primitive_type)
    orb.coef_ = orb.coefgen('ones')
    bas = orb.to_griddata(dr=dr)
    
    psp = PspParser(fn)
    return [_solve(vgrid=psp.r(), 
                   v=psp.vlocal(), 
                   rgrid=np.linspace(0, rcut, int(rcut/dr)+1), 
                   bas=bas[l_], 
                   l=l_, 
                   nroot=nz)[1].T.tolist() for l_, nz in zip(l, nzeta)]
...
class Spillage:
    ...
    def opt(self, 
        ...
        ----------
            ...
            fix_components : list of list of int
                indexes of zeta functions that are fixed during optimization.
        ...
        bounds = bounds or [(-1.0, 1.0) for _ in c0] # default bounds

        res = minimize(f, c0, jac=True, method='L-BFGS-B',
                       bounds=np.array(flatten(bounds)), options=options)
...
class _TestSpillage(unittest.TestCase):
    ...
    def test_initgen_file(self):
        ...
        coef = initgen_file(forb, [1, 1, 0], 100.0, 10.0)
        ...
        griddata = build_reduced(coef, 10.0, 0.01 * np.arange(1001), True)
    ...
    def test_jy_opt(self):
        ...
        coef_lvl1_init = [[[coef_init[0][0]],
                           [coef_init[1][0]]]]
        coef_lvl1 = orbgen.opt(coef_lvl1_init, None, None, iconfs, ibands,
                               options, nthreads)[0]
        coef_tot = coef_lvl1
        ...
        coef_lvl2_init = [[[coef_init[0][1]],
                           [coef_init[1][1]],
                           [coef_init[2][0]]]]
        coef_lvl2 = orbgen.opt(coef_lvl2_init, coef_lvl1, None, iconfs, ibands,
                               options, nthreads)[0]
        ...
        coef_lvl3_init = [[[coef_init[0][2]],
                           [coef_init[1][2]],
                           [coef_init[2][1]]]]
        coef_lvl3 = orbgen.opt(coef_lvl3_init, coef_tot, None, iconfs, ibands,
                               options, nthreads)[0]
