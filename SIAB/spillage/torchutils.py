...
def minimize(f, c, method, maxiter, disp, ndisp, **kwargs):
    ...
    **kwargs : dict
    ...
    if disp:
        ...
        logging.info(f'{"method":>10s} {"step":>8s} {"loss (spillage)":>20s} {"time/s":>10s}')
    ...
    for i in range(maxiter):
        ...
        if disp and i % (maxiter//ndisp) == 0:
            dt = time() - t0
            t0 = time()
            logging.info(f'{method:>10s} {i:>8d} {loss.item():>20.10e} {dt:>10.2f}')
        # autodiff
        loss.backward()
        optimizer.step()
    if disp:
        logging.info(f'{method:>10s} {maxiter:>8d} {loss.item():>20.10e} {time()-t0:>10.2f}\n')
...
def _t_jy2ao(coef: torch.Tensor, natom: int, nzeta: list, nbes: list) -> torch.Tensor:
    ...
    '''
    OrbgenAssert(len(natom) == len(nbes),
                 'natom and nbes must have the same length',
                 ValueError)
